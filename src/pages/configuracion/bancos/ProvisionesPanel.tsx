...
export default function ProvisionesPanel() {
  ...
  const label: CSSProperties = {
    fontFamily: FONT.sans, fontSize: 10, letterSpacing: 1.2,
    textTransform: 'uppercase', color: T.textTertiary, marginBottom: 4, fontWeight: 500,
  }
  const input: CSSProperties = {
    width: 120, padding: '7px 10px', background: T.bgSurfaceAlt, color: T.textPrimary,
    border: `1px solid ${T.borderDefault}`, borderRadius: 6, fontFamily: FONT.sans, fontSize: 13,
    outline: 'none',
  }
  const btnPrimary: CSSProperties = {
    padding: '8px 16px', borderRadius: 6, border: 'none',
    background: T.brandAccent, color: '#fff',
    fontFamily: FONT.sans, fontSize: 11, letterSpacing: 1,
    textTransform: 'uppercase', fontWeight: 600, cursor: 'pointer',
  }
  const btnGhost: CSSProperties = {
    padding: '8px 16px', borderRadius: 6, border: `0.5px solid ${T.borderDefault}`,
    ...
    <>
        ...
        <div style={{ padding: '14px 22px 18px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ color: T.textTertiary, fontFamily: FONT.sans, fontSize: 13 }}>€</span>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div style={{ fontFamily: FONT.sans, fontSize: 24, fontWeight: 500, color: T.textPrimary, letterSpacing: '-0.01em' }}>
      ...
      <ConfigGroupCard title="Resumen provisiones" subtitle={loading ? 'cargando…' : `${provHist.reduce((a, b) => a + b.count, 0)} registros`}>
        <div style={{ padding: '12px 22px 18px' }}>
          {provHist.length === 0 ? (
            <div style={{ color: T.textTertiary, fontFamily: FONT.sans, fontSize: 13, padding: 12 }}>Sin provisiones</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  <th style={{ textAlign: 'left', padding: '6px 8px', fontFamily: FONT.sans, fontSize: 10, color: T.textTertiary, letterSpacing: 1.2, textTransform: 'uppercase', fontWeight: 500, borderBottom: `1px solid ${T.borderDefault}` }}>Tipo</th>
                  <th style={{ textAlign: 'right', padding: '6px 8px', fontFamily: FONT.sans, fontSize: 10, color: T.textTertiary, letterSpacing: 1.2, textTransform: 'uppercase', fontWeight: 500, borderBottom: `1px solid ${T.borderDefault}` }}>Registros</th>
                  <th style={{ textAlign: 'right', padding: '6px 8px', fontFamily: FONT.sans, fontSize: 10, color: T.textTertiary, letterSpacing: 1.2, textTransform: 'uppercase', fontWeight: 500, borderBottom: `1px solid ${T.borderDefault}` }}>Total</th>
                  <th style={{ textAlign: 'right', padding: '6px 8px', fontFamily: FONT.sans, fontSize: 10, color: T.textTertiary, letterSpacing: 1.2, textTransform: 'uppercase', fontWeight: 500, borderBottom: `1px solid ${T.borderDefault}` }}>Pendiente</th>
                </tr>
              </thead>
              <tbody>
                {provHist.map(p => (
                  <tr key={p.tipo}>
                    <td style={{ padding: '8px', fontFamily: FONT.sans, fontSize: 13, color: T.textPrimary, borderBottom: `0.5px solid ${T.borderDefault}` }}>
                      {p.tipo === 'IVA_TRIM' ? 'IVA trimestral' : p.tipo === 'IRPF_ALQ' ? 'IRPF alquiler mensual' : p.tipo}
                    </td>
                    <td style={{ padding: '8px', textAlign: 'right', fontFamily: FONT.sans, fontSize: 13, color: T.textPrimary, borderBottom: `0.5px solid ${T.borderDefault}`, fontVariantNumeric: 'tabular-nums' }}>
                      {p.count}
                    </td>
                    <td style={{ padding: '8px', textAlign: 'right', fontFamily: FONT.sans, fontSize: 13, color: T.textPrimary, fontWeight: 500, borderBottom: `0.5px solid ${T.borderDefault}`, fontVariantNumeric: 'tabular-nums' }}>
                      {fmtEur(p.total)}
                    </td>
                    <td style={{ padding: '8px', textAlign: 'right', fontFamily: FONT.sans, fontSize: 13, color: T.brandAccent, fontWeight: 500, borderBottom: `0.5px solid ${T.borderDefault}`, fontVariantNumeric: 'tabular-nums' }}>
                      {fmtEur(p.pendiente)}
                    </td>
                  </tr>
                ))}
