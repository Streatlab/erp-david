...
export default function ReglasPanel() {
  ...
  const td: CSSProperties = { padding: '10px 14px', fontFamily: FONT.sans, fontSize: 13, color: T.textPrimary }
      ...
      <ConfigGroupCard title="Reglas de asignación automática" subtitle={`${reglas.length} reglas`}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', fontSize: 13, whiteSpace: 'nowrap', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderTop: `0.5px solid ${T.borderDefault}`, borderBottom: `0.5px solid ${T.borderDefault}`, background: T.bgSurfaceAlt }}>
              ...
              {reglas.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: '40px 22px', textAlign: 'center' }}>
                    <Wand2 size={32} color={T.textTertiary} style={{ marginBottom: 12 }} />
                ...
                <tr key={r.id} style={{ borderBottom: `0.5px solid ${T.borderDefault}` }}>
                  <td
                    onClick={() => open(r)}
                  ...
                  <td onClick={() => open(r)} style={{ ...td, cursor: 'pointer' }}>
                    {r.categoria_codigo ? (
                      <>
                        <span style={{ fontFamily: FONT.sans, fontSize: 11, color: T.textSecondary, marginRight: 8, letterSpacing: 0.5 }}>{r.categoria_codigo}</span>
                        <span>{nombrePorCodigo[r.categoria_codigo] ?? '—'}</span>
                      </>
                    ) : '—'}
                  </td>
                  <td onClick={() => open(r)} style={{ ...td, textTransform: 'capitalize', color: T.textSecondary, cursor: 'pointer' }}>{r.asigna_como}</td>
                  ...
                  <td onClick={() => open(r)} style={{ ...td, textAlign: 'right', cursor: 'pointer', color: T.textSecondary }}>{r.prioridad}</td>
                </tr>
              ))}
            </tbody>
          </table>
