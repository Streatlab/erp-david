...
export function PeriodDropdown({ value, onChange, customRange }: Props) {
  ...
  return (
    <div className="relative inline-block">
      <button
        onClick={() => setOpen(o => !o)}
        className="text-[var(--terra-500)] text-xs font-medium underline decoration-dotted underline-offset-4 hover:text-[#901A1E]"
      >{LABELS[value]} ▾</button>
      {open && (
        <div className="absolute right-0 mt-2 w-[220px] bg-white border border-[#E9E1D0] rounded-lg shadow-lg z-20 p-1">
          {(['30d','mes_curso','60d','90d','custom'] as Periodo[]).map(p => (
            <button
              ...
              className={`block w-full text-left px-3 py-2 text-xs rounded ${value === p ? 'bg-[#FCE0E2] text-[var(--terra-500)] font-medium' : 'hover:bg-[#F1EADD]'}`}
            >{LABELS[p]}</button>
          ))}
          {value === 'custom' && (
            <div className="p-2 border-t border-[#E9E1D0] mt-1 space-y-2">
              ...
              <button
                ...
                disabled={!from || !to}
                className="w-full px-3 py-1.5 text-xs bg-[var(--terra-500)] text-white rounded-md hover:bg-[#901A1E] disabled:opacity-50"
              >Aplicar</button>
            </div>
          )}
        </div>
